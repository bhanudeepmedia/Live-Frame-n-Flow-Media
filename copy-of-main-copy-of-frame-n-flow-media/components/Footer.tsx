...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-[#0a0a0a] border-t border-white/10 relative overflow-hidden z-20">
      ...
      <div className="container mx-auto px-6 py-16 md:py-20 relative z-20">
            ...
            <nav className="flex flex-wrap gap-x-8 gap-y-6 justify-start md:justify-end max-w-3xl">
              {FOOTER_ITEMS.map((item) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `font-display font-medium text-lg md:text-xl transition-all flex items-center group relative pr-8 ${isActive ? 'text-white' : 'text-white/50 hover:text-accent'
                    }`
                  }
                >
                  {item.label}
        ...
        <div className="mt-20 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center text-white/20 text-xs font-mono uppercase tracking-widest">
          ...
          <div className="flex gap-8 mt-4 md:mt-0">
            <NavLink to="/offers/website-25k" onClick={handleLinkClick} className="text-accent font-bold hover:scale-105 transition-all">₹25,000 Website Offer</NavLink>
